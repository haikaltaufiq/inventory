...
<div class="px-5">
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-6">
        ...
        <div class="rounded-2xl bg-white p-5 shadow-sm">
            <p class="text-sm text-slate-500">Tanpa Pemodal</p>
            <h3
                class="mt-1 text-2xl font-semibold {{ $summary['total_tanpa_pemodal'] > 0 ? 'text-rose-600' : 'text-slate-900' }}">
                {{ number_format($summary['total_tanpa_pemodal']) }}
            </h3>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="transition hover:bg-slate-50/80">
                        ...
                        <td class="px-3 py-3.5 align-middle text-slate-700">
                            @if ($row->pemodal_name)
                            <div class="truncate font-semibold leading-5 text-slate-900">{{ $row->pemodal_name }}</div>
                            @if ($row->pemodal_role)
                            <div class="mt-0.5 truncate text-[11px] capitalize text-slate-400">{{ $row->pemodal_role }}</div>
                            @endif
                            @else
                        ...
                        <td class="px-3 py-3.5 align-middle font-medium text-slate-900">
                            ...
                            <div class="mt-0.5 truncate text-[11px] text-slate-500">{{ $row->category_name ?: 'Tanpa kategori' }}
                            </div>
                        </td>
                        <td class="px-3 py-3.5 align-middle text-slate-700">
                        ...
                        <td class="whitespace-nowrap px-3 py-3.5 text-right align-middle font-semibold tabular-nums text-slate-900">
                        ...
                        <td class="whitespace-nowrap px-3 py-3.5 text-right align-middle font-semibold tabular-nums text-amber-600">
                            {{ number_format((int) $row->sold_qty) }}
                        </td>
                        <td class="whitespace-nowrap px-3 py-3.5 text-right align-middle font-semibold tabular-nums text-emerald-600">
                        ...
                        <td class="px-3 py-3.5 align-middle text-slate-700">
                            ...
                            <div class="flex min-w-0 items-center gap-1.5">
                                <span class="truncate font-medium text-slate-800">{{ $firstSeller['name'] }}</span>
    ...
    <x-modal id="{{ $detailModalId }}" title="Detail Stok" size="lg">
        <div class="space-y-5">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">{{ $row->category_name ?: 'Tanpa kategori' }} - {{ $row->condition ?: '-' }}</p>
            </div>
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200">
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        <tr>
                            <td class="whitespace-nowrap px-4 py-3 font-semibold tabular-nums">Rp {{ number_format((float) $row->modal, 0, ',', '.') }}</td>
                            <td class="whitespace-nowrap px-4 py-3 font-semibold tabular-nums">Rp {{ number_format((float) $row->harga_jual, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right font-semibold tabular-nums">{{ number_format((int) $row->stock_awal) }}</td>
                            <td class="px-4 py-3 text-right font-semibold tabular-nums text-amber-600">{{ number_format((int) $row->sold_qty) }}</td>
                            <td class="px-4 py-3 text-right font-semibold tabular-nums text-emerald-600">{{ number_format((int) $row->stock_ready) }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-right font-semibold tabular-nums">Rp {{ number_format((float) $row->total_modal, 0, ',', '.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                ...
                <div class="mt-3 overflow-hidden rounded-xl border border-slate-200">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr>
                                <td class="px-4 py-3 font-medium text-slate-800">{{ $seller['name'] }}</td>
                                <td class="px-4 py-3 text-right font-semibold tabular-nums text-slate-900">{{ number_format($seller['qty']) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
