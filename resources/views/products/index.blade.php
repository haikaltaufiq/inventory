    ...
    <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
            <div class="flex bg-white border border-slate-200 rounded-xl p-1 shadow-sm">
                <button class="px-3 py-1.5 text-xs font-medium hover:bg-slate-50 rounded-lg transition">Export PDF</button>
                <button class="px-3 py-1.5 text-xs font-medium hover:bg-slate-50 rounded-lg transition border-l border-slate-100">CSV</button>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            ...
            <h3 class="text-2xl font-bold mt-2 text-slate-800">
                <span class="text-sm font-medium text-slate-400">Rp</span> {{ number_format($summary['nilai_inv'], 0, ',', '.') }}
            </h3>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            ...
            <div class="flex items-center gap-2 mt-2">
                ...
                <span class="text-[10px] px-2 py-0.5 bg-red-50 text-red-600 rounded-full font-medium animate-pulse border border-red-100">
                    Butuh Restock
                </span>
                @else
        ...
        <form action="{{ route('products.index') }}" method="GET" class="flex flex-wrap md:flex-nowrap gap-3">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                    ...
                    class="w-full pl-11 pr-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-4 focus:ring-slate-900/5 focus:border-slate-400 outline-none transition">
            </div>
            ...
            <button type="submit" class="px-6 py-2.5 bg-slate-900 text-white font-medium rounded-xl text-sm hover:bg-slate-800 transition">
                Terapkan Filter
            </button>

            @if(request()->anyFilled(['search', 'category_id']))
            <a href="{{ route('products.index') }}" class="px-4 py-2.5 bg-slate-100 text-slate-500 rounded-xl text-sm hover:bg-slate-200 transition flex items-center justify-center">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    @php
                    $colorMap = ['bg-blue-500', 'bg-purple-500', 'bg-emerald-500', 'bg-amber-500', 'bg-pink-500'];
                    @endphp
                    ...
                    <tr class="hover:bg-slate-50/80 transition align-top">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="text-[10px] text-slate-400 mt-1 font-mono">#PRD-{{ str_pad($product->id, 4, '0', STR_PAD_LEFT) }}</div>
                        </td>

                        {{-- KATEGORI --}}
                        <td class="px-6 py-4">
                            @if($product->category)
                            <span class="px-3 py-1 rounded-full text-[10px] font-semibold bg-slate-100 text-slate-600  uppercase tracking-wider">
                                {{ $product->category->name }}
                            </span>
                            @else
                            <span class="px-3 py-1 rounded-full text-[10px] font-semibold bg-slate-50 text-slate-400 border border-dashed border-slate-200 uppercase tracking-wider">
                                No Category
                            </span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="h-7 flex items-center gap-2">
                                    ...
                                    <span class="text-slate-600 text-[11px] font-medium truncate max-w-30">{{ $supplier->nama_supplier }}</span>
                                </div>
                                @endforeach
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="h-7 flex items-center justify-center">
                                    <span class="text-[11px] font-bold px-2.5 py-0.5 rounded-lg {{ $supplier->pivot->stock <= 5 ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-700' }}">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="h-7 flex flex-col justify-center">
                                    <span class="text-[11px] font-medium text-slate-400">Rp {{ number_format($supplier->pivot->harga_beli, 0, ',', '.') }}</span>
                                </div>
                                @endforeach
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-3">
                                @foreach($product->suppliers as $supplier)
                                <div class="h-7 flex flex-col justify-center">
                                    <span class="text-[11px] font-bold text-slate-900">Rp {{ number_format($supplier->pivot->harga_jual_manual, 0, ',', '.') }}</span>
                                </div>
                                @endforeach
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-3">
                                ...
                                @php $color = $colorMap[$index % count($colorMap)]; @endphp
                                <div class="h-7 flex items-center gap-2">
                                    <span class="px-2.5 py-1 rounded-full text-[10px] font-medium  {{ $supplier->pivot->condition == 'New' ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600' }}">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="h-7 flex items-center justify-center">
                                    ...
                                    <span class="px-2.5 py-1 rounded-full text-[10px] font-medium bg-emerald-100 text-emerald-600">
                                        Aman
                                    </span>
                                    @elseif($supplier->pivot->stock > 0)
                                    <span class="px-2.5 py-1 rounded-full text-[10px] font-medium bg-yellow-100 text-yellow-600">
                                        Menipis
                                    </span>
                                    @else
                                    <span class="px-2.5 py-1 rounded-full text-[10px] font-medium bg-rose-100 text-rose-600">
                                        Kosong
                                    </span>
                                    @endif
                                </div>
