...
@section('content')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-slate-600 mb-1">Harga Patokan
                                        (Baru)</label>
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm">{{ $product->description }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div id="specs-container" class="space-y-3">
                            @forelse($product->specifications as $index => $spec)
                                <div class="flex gap-3 spec-row">
                                    <input type="text" name="specs[{{ $index }}][key]"
                                        value="{{ $spec->spec_key }}" placeholder="Key"
                                        class="w-1/3 px-4 py-2 rounded-xl border border-slate-200 text-sm">
                            ...
                            @empty
                                <div class="flex gap-3 spec-row">
                                    <input type="text" name="specs[0][key]" placeholder="Contoh: Socket"
                                        class="w-1/3 px-4 py-2 rounded-xl border border-slate-200 text-sm">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm p-6">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button type="button" onclick="addSupplier()"
                                class="px-4 py-2 bg-slate-900 text-white text-xs rounded-xl hover:bg-slate-800">+ Tambah
                                Supplier</button>
                        </div>
                        <div id="suppliers-container" class="space-y-4">
                            @foreach ($product->suppliers as $index => $sup)
                                <div class="supplier-row bg-slate-50 border border-slate-200 rounded-2xl p-5"
                                    data-index="{{ $index }}">
                                    <div class="flex justify-between items-center mb-4">
                                        <span class="text-sm font-semibold text-slate-700">Supplier
                                            #{{ $index + 1 }}</span>
                                        ...
                                        <div class="md:col-span-1">
                                            ...
                                            <select name="suppliers[{{ $index }}][supplier_id]"
                                                ...
                                                @foreach ($suppliers as $s)
                                                    <option value="{{ $s->id }}"
                                                        {{ $sup->id == $s->id ? 'selected' : '' }}>{{ $s->nama_supplier }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div>
                                            ...
                                            <select name="suppliers[{{ $index }}][condition]"
                                                ...
                                                <option value="Refurbished"
                                                    {{ $sup->pivot->condition == 'Refurbished' ? 'selected' : '' }}>
                                                    Refurbished</option>
                                            </select>
                                        </div>
                                        <div>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-2xl shadow-sm p-6 sticky top-5">
                        ...
                        <div class="space-y-3">
                            <button type="submit"
                                class="w-full py-3 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition">Update
                                Produk</button>
                            <a href="{{ route('products.index') }}"
                                class="block w-full py-3 rounded-xl bg-slate-100 text-slate-700 text-sm text-center hover:bg-slate-200 transition">Batal</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        let specIndex = {{ $product->specifications->count() ?? 0 }};
        let supplierIndex = {{ $product->suppliers->count() ?? 0 }};
        ...
        <div class="supplier-row bg-slate-50 border border-slate-200 rounded-2xl p-5 mt-4" data-index="${supplierIndex}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {{-- Lanjutin sisa inputnya sesuai code sebelumnya --}}
            </div>
        </div>
        `);
            supplierIndex++;
