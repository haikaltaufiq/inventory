...
@section('content')
    <div class="px-4 pb-10 lg:px-5" x-data="specPresets()">
        ...
        @if (session('success'))
            <div
                class="mb-5 flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
        ...
        @if (session('error'))
            <div
                class="mb-5 flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @foreach ($sections as $section)
                <div x-show="sectionVisible(@js($section))"
                    ...
                    <div class="flex items-start justify-between gap-3 border-b border-slate-100 px-4 py-3.5">
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                ...
                                @if ($section['required'])
                                    <span
                                        class="rounded-full bg-slate-900 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.14em] text-white">
                                        Required
                                    </span>
                                @else
                                    <span
                                        class="rounded-full bg-slate-100 px-2 py-0.5 text-[9px] font-medium uppercase tracking-[0.14em] text-slate-500">
                                        Optional
                                    </span>
                                @endif
                            </div>
                            <div class="mt-0.5 flex items-center gap-1.5 text-[11px] text-slate-400">
                                <code
                                    class="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] text-slate-600">{{ $section['key'] }}</code>
                                <span>·</span>
                    ...
                    @if ($section['hint'])
                        <div
                            class="border-b border-slate-100 bg-slate-50/60 px-4 py-2 text-[11px] leading-4 text-slate-500">
                            {{ $section['hint'] }}
                        </div>
                    @endif
                    ...
                    <div class="px-4 py-3">
                        ...
                        <div class="mt-3 border-t border-slate-100 pt-3" x-data="{ open: false, value: '' }">
                            ...
                            <div x-show="open" x-cloak class="mt-2 flex gap-2">
                                <form action="{{ route('spec-presets.store') }}" method="POST" class="flex flex-1 gap-2">
                                    @csrf
                                    <input type="hidden" name="spec_key" value="{{ $section['key'] }}">
                                    <input type="text" name="spec_value" x-model="value"
